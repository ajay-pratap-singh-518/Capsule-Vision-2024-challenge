...
class FocalLoss(torch.nn.Module):
    ...
    def forward(self, inputs, targets):
        if inputs.size(0) != targets.size(0):# Ensure inputs and targets have the same shape
...
class attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_map = self.softmax(attention_scores)  # (batch_size, h * w, h * w)# Apply softmax to get attention map
...
val_transform = transforms.Compose([  # Define transformations for validation
...
def create_val_dataloader(val_dir, batch_size=32):# Function to create validation data loader
...
def validate_model(val_loader, model, criterion, output_folder='atten_focal_equal/validation_results_mobv3'):# Validation function
...
def save_predictions_to_excel(predictions, labels, logits, class_names, output_path):
    ...
    if not (len(predictions) == len(labels) == len(logits)): # Ensure all inputs have the same length
...
if __name__ == "__main__":
    ...
    criterion = FocalLoss(alpha=0.25, gamma=2.0)# Define the loss function (consistent with training)
    validate_model(val_loader, model, criterion)# Perform validation
